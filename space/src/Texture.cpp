...
GLuint Texture::loadTexture(std::string name)
{

	std::string img_path = std::string(TEXDIR) + name + std::string(TEX_SUFFIX);
...
GLuint Texture::loadNRNTexture(std::string img_path)
{
	if(m_texMap.find(img_path) != m_texMap.end())
	{
		//printf("%s was already loaded. Returning existing texture handle.\n", img_path);
